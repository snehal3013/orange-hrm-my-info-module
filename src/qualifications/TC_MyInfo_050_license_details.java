...
public class TC_MyInfo_050_license_details {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Sheet s = wb.getSheet("TC_MyInfo_050");
			...
			for (int i = 0; i <= lictype.size() - 1; i++) {
				...
				if (lictype.get(i).getText().contains(s.getCell(1, 16).getContents())) {
			...
			Thread.sleep(2000);
			issueddate.sendKeys(s.getCell(1, 17).getContents());
			Thread.sleep(2000);
			// expiry date
			...
			Thread.sleep(2000);
			expdate.sendKeys(s.getCell(1, 18).getContents());
			Thread.sleep(2000);
			// save
