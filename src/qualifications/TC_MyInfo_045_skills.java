...
public class TC_MyInfo_045_skills {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Sheet s = wb.getSheet("TC_MyInfo_045");

			driver.get(s.getCell(1, 0).getContents());
			Thread.sleep(2000);
			driver.manage().window().maximize();
			// login
			driver.findElement(By.xpath(s.getCell(1, 3).getContents())).sendKeys(s.getCell(1, 18).getContents());
			driver.findElement(By.xpath(s.getCell(1, 4).getContents())).sendKeys(s.getCell(1, 19).getContents());
			...
			for (int i = 0; i <= skill.size() - 1; i++) {
				...
				if (skill.get(i).getText().contains(s.getCell(1, 20).getContents())) {
			...
			// year
			driver.findElement(By.xpath(s.getCell(1, 11).getContents())).sendKeys(s.getCell(1, 21).getContents());
			Thread.sleep(2000);
			// save
			...
			Thread.sleep(2000);
			List<WebElement> skill1 = driver.findElements(By.xpath(s.getCell(1, 15).getContents()));
			Thread.sleep(2000);
			for (int i = 0; i <= skill1.size() - 1; i++) {

				if (skill1.get(i).getText().contains(s.getCell(1, 22).getContents())) {
			...
			// year
			driver.findElement(By.xpath(s.getCell(1, 16).getContents())).sendKeys(s.getCell(1, 23).getContents());
			Thread.sleep(2000);
			// save
